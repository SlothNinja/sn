...
func newEloDefault(uKey *datastore.Key) *Elo {
	t := time.Now()
	return &Elo{
		Key:       newCurrentEloKey(uKey),
...
func newCurrentEloKey(uKey *datastore.Key) *datastore.Key {
	return datastore.NameKey(eloKind, "current", uKey)
}

func newEloKey(uKey *datastore.Key) *datastore.Key {
	return datastore.IncompleteKey(eloKind, uKey)
}
...
func (cl *EloClient) GetByUserKeys(c *gin.Context, uKeys []*datastore.Key) ([]*Elo, error) {
	elos := make([]*Elo, len(uKeys))
	ks := make([]*datastore.Key, len(uKeys))
	for i := range elos {
		ks[i] = newCurrentEloKey(uKeys[i])
	}
	err := cl.DS.GetMulti(c, ks, elos)
	return elos, err
}

func (cl *EloClient) GetByUserKey(c *gin.Context, uKey *datastore.Key) (*Elo, error) {
	elos, err := cl.GetByUserKeys(c, []*datastore.Key{uKey})
	if err != nil {
		return nil, err
	}
	if len(elos) != 1 {
...
type eloMap map[*datastore.Key]*Elo
type PlacesMap map[*datastore.Key]int

const notFound = 0

func updateEloFor(uKey1 *datastore.Key, elos eloMap, places PlacesMap) int {
	elo := elogo.NewElo()
	for uKey2 := range elos {
		if uKey1.Equal(uKey2) {
			continue
		}
		score := 0.0
		if places[uKey1] == places[uKey2] {
			score = 0.5
		}
		if places[uKey1] < places[uKey2] {
			score = 1
		}
		elos[uKey1].Rating = elo.Rating(elos[uKey1].Rating, elos[uKey2].Rating, score)
	}
	return elos[uKey1].Rating
}

// Update pulls current Elo from db and provides rating updates and deltas per results for users associated with uKeys.
